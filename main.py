...
def getlink():
    url_link = 'https://www.gd.gov.cn/gkmlpt/policy'
    headers = {
    ...
    rows=html.xpath('/html/body/div[2]')
    #print(rows)
    for row in rows:
...
def crawl_policy_info(url):
    ...
    for policy in policy_list:
        ...
        # 将政策文本保存为txt文件
        with open(f'./text/{title}.txt', 'a', encoding='utf-8') as file:
            for line in text:
                file.write(line + '\n')
...
url=['https://www.gd.gov.cn/gkmlpt/content/4/4199/post_4199084.html#8',
     ...
     'https://www.gd.gov.cn/gkmlpt/content/4/4151/post_4151766.html#8']
